...
parser.add_argument('--data_path',
                    default='/scratch/local/ssd/weidi/voxceleb2/dev/wav',
                    type=str)
...
parser.add_argument('--aggregation_mode',
                    default='gvlad',
...
parser.add_argument(
    ...
    type=int,
    help=
    'pick hard samples from (ohem_level * batch_size) proposals, must be > 1')
global args
...
def main():
    ...
    trnlist, trnlb, vallist, vallb = more_tts.get_moretts_datalist()
    print('--------------------- load datalist done')
...
def step_decay(epoch):
    ...
    stage1, stage2, stage3 = int(half_epoch * 0.5), int(half_epoch *
                                                        0.8), half_epoch
...
def set_path(args):
    ...
    if args.aggregation_mode == 'avg':
        exp_path = os.path.join(
            ...
            'lr{args.lr}_bdim{args.bottleneck_dim}_ohemlevel{args.ohem_level}'.
            format(date, args=args))
